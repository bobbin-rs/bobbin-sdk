...
pub fn init() {
    ...
    clock_init::enable_pll_hse_mode(12, 432, 0b00, 9, true);
}
...
static mut HSE_HZ: Hz = Hz::from_num(12_000_000);
...
pub mod clock_init {
    ...
    pub fn enable_pll_hse_mode(m: u32, n: u32, p: u32, q: u32, bypass: bool) {
        let rcc = rcc::RCC;
        ...
        rcc.with_cr(|r| r.set_hseon(1).set_hsebyp(bypass));
    ...
    pub fn enable_pll_hse_mode_orig(m: u32, n: u32, p: u32, q: u32) {
        let rcc = rcc::RCC;
        let flash = flash::FLASH;
        let pwr = pwr::PWR;
