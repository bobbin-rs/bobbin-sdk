...
impl Dma {
  ...
  #[inline] pub fn cselr_ptr(&self) -> *const u32 { 
     ((self.0 as usize) + 0xa8) as *const u32
  }
  #[inline] pub fn cselr_mut(&self) -> *mut u32 { 
  ...
  #[inline] pub fn set_cselr(&self, value: Cselr) -> &Self {
     unsafe {
...
impl Cselr {
  #[inline] pub fn cs(&self, index: usize) -> u32 {
     assert!(index < 7);
     let shift: usize = 0 + (index << 2);
     ((self.0 as u32) >> shift) & 0xf // [3:0]
  }
  #[inline] pub fn set_cs(mut self, index: usize, value: u32) -> Self {
     assert!(index < 7);
     assert!((value & !0xf) == 0);
     let shift: usize = 0 + (index << 2);
     self.0 &= !(0xf << shift);
