...
pub fn init(sysclk_hz: u32) {
    init_handler(sysclk_hz, Some(default_systick_handler))
}

pub fn init_handler(sysclk_hz: u32, handler: Option<Handler>) {
    set_handler(handler);
...
pub fn set_handler(handler: Option<Handler>) {
    ::chip::vtor::SYSTICK.set_handler(handler)
}

//#[export_name = "_systick"]
pub extern "C" fn default_systick_handler() {
    unsafe {
        TICK_COUNT_LO += 1;
        if TICK_COUNT_LO == 0 {
            TICK_COUNT_HI += 1;
