...
periph!(FlashPeriph, FLASH, Flash, 0x40023c00);
...
#[doc="Get the *const pointer for the OPTCR register."]
   #[inline] fn optcr_ptr(&self) -> *const u32 { 
       <Self as Base>::addr(&self, 0x14)
   }
#[doc="Get the *mut pointer for the OPTCR register."]
   #[inline] fn optcr_mut(&self) -> *mut u32 { 
       <Self as Base>::addr(&self, 0x14)
   }
#[doc="Read the OPTCR register."]
   #[inline] fn optcr(&self) -> Optcr { 
      unsafe {
         Optcr(::core::ptr::read_volatile((self.base() + 0x14) as *const u32))
      }
   }
#[doc="Write the OPTCR register."]
   #[inline] fn set_optcr<F: FnOnce(Optcr) -> Optcr>(&self, f: F) -> &Self {
      let value = f(Optcr(0));
      unsafe {
...
#[doc="Modify the OPTCR register."]
   #[inline] fn with_optcr<F: FnOnce(Optcr) -> Optcr>(&self, f: F) -> &Self {
      let tmp = unsafe {
         Optcr(::core::ptr::read_volatile((self.base() + 0x14) as *const u32))
      };
      let value = f(tmp);
