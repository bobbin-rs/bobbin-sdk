...
periph!(SyscfgPeriph, SYSCFG, Syscfg, 0x40013800);

#[doc="System configuration controller"]
pub trait SyscfgPeriph : Base {
#[doc="Get the *const pointer for the MEMRM register."]
   #[inline] fn memrm_ptr(&self) -> *const u32 { 
       <Self as Base>::addr(&self, 0x0)
   }
#[doc="Get the *mut pointer for the MEMRM register."]
   #[inline] fn memrm_mut(&self) -> *mut u32 { 
       <Self as Base>::addr(&self, 0x0)
   }
#[doc="Read the MEMRM register."]
   #[inline] fn memrm(&self) -> Memrm { 
      unsafe {
         Memrm(::core::ptr::read_volatile((self.base() + 0x0) as *const u32))
      }
   }
#[doc="Write the MEMRM register."]
   #[inline] fn set_memrm<F: FnOnce(Memrm) -> Memrm>(&self, f: F) -> &Self {
      let value = f(Memrm(0));
      unsafe {
...
#[doc="Modify the MEMRM register."]
   #[inline] fn with_memrm<F: FnOnce(Memrm) -> Memrm>(&self, f: F) -> &Self {
      let tmp = unsafe {
         Memrm(::core::ptr::read_volatile((self.base() + 0x0) as *const u32))
      };
      let value = f(tmp);
...
#[doc="Get the *const pointer for the PMC register."]
   #[inline] fn pmc_ptr(&self) -> *const u32 { 
       <Self as Base>::addr(&self, 0x4)
   }
#[doc="Get the *mut pointer for the PMC register."]
   #[inline] fn pmc_mut(&self) -> *mut u32 { 
       <Self as Base>::addr(&self, 0x4)
   }
#[doc="Read the PMC register."]
   #[inline] fn pmc(&self) -> Pmc { 
      unsafe {
         Pmc(::core::ptr::read_volatile((self.base() + 0x4) as *const u32))
      }
   }
#[doc="Write the PMC register."]
   #[inline] fn set_pmc<F: FnOnce(Pmc) -> Pmc>(&self, f: F) -> &Self {
      let value = f(Pmc(0));
      unsafe {
...
#[doc="Modify the PMC register."]
   #[inline] fn with_pmc<F: FnOnce(Pmc) -> Pmc>(&self, f: F) -> &Self {
      let tmp = unsafe {
         Pmc(::core::ptr::read_volatile((self.base() + 0x4) as *const u32))
      };
      let value = f(tmp);
...
#[doc="Get the *const pointer for the CMPCR register."]
   #[inline] fn cmpcr_ptr(&self) -> *const u32 { 
       <Self as Base>::addr(&self, 0x20)
   }
#[doc="Get the *mut pointer for the CMPCR register."]
   #[inline] fn cmpcr_mut(&self) -> *mut u32 { 
       <Self as Base>::addr(&self, 0x20)
   }
#[doc="Read the CMPCR register."]
   #[inline] fn cmpcr(&self) -> Cmpcr { 
      unsafe {
         Cmpcr(::core::ptr::read_volatile((self.base() + 0x20) as *const u32))
      }
   }
