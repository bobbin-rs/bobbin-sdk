...
#[doc="Get the *const pointer for the TAFCR register."]
   #[inline] fn tafcr_ptr(&self) -> *const u32 { 
       <Self as Base>::addr(&self, 0x40)
   }
#[doc="Get the *mut pointer for the TAFCR register."]
   #[inline] fn tafcr_mut(&self) -> *mut u32 { 
       <Self as Base>::addr(&self, 0x40)
   }
#[doc="Read the TAFCR register."]
   #[inline] fn tafcr(&self) -> Tafcr { 
      unsafe {
         Tafcr(::core::ptr::read_volatile((self.base() + 0x40) as *const u32))
      }
   }
#[doc="Write the TAFCR register."]
   #[inline] fn set_tafcr<F: FnOnce(Tafcr) -> Tafcr>(&self, f: F) -> &Self {
      let value = f(Tafcr(0));
      unsafe {
...
#[doc="Modify the TAFCR register."]
   #[inline] fn with_tafcr<F: FnOnce(Tafcr) -> Tafcr>(&self, f: F) -> &Self {
      let tmp = unsafe {
         Tafcr(::core::ptr::read_volatile((self.base() + 0x40) as *const u32))
      };
      let value = f(tmp);
...
#[doc="Get the *const pointer for the BKPR register."]
   #[inline] fn bkpr_ptr<I: Into<bits::R32>>(&self, index: I) -> *const u32 { 
      let index: bits::R32 = index.into();
      let index: usize = index.value() as usize;
      <Self as Base>::addr(&self, 0x50 + (index << 2))
   }
#[doc="Get the *mut pointer for the BKPR register."]
   #[inline] fn bkpr_mut<I: Into<bits::R32>>(&self, index: I) -> *mut u32 { 
      let index: bits::R32 = index.into();
      let index: usize = index.value() as usize;
      <Self as Base>::addr(&self, 0x50 + (index << 2))
   }
#[doc="Read the BKPR register."]
   #[inline] fn bkpr<I: Into<bits::R32>>(&self, index: I) -> Bkpr { 
      let index: bits::R32 = index.into();
      let index: usize = index.value() as usize;
      unsafe {
...
#[doc="Write the BKPR register."]
   #[inline] fn set_bkpr<I: Into<bits::R32>, F: FnOnce(Bkpr) -> Bkpr>(&self, index: I, f: F) -> &Self {
      let index: bits::R32 = index.into();
      let index: usize = index.value() as usize;
      let value = f(Bkpr(0));
      unsafe {
...
#[doc="Modify the BKPR register."]
   #[inline] fn with_bkpr<I: Into<bits::R32> + Copy, F: FnOnce(Bkpr) -> Bkpr>(&self, index: I, f: F) -> &Self {
      let index: bits::R32 = index.into();
      let index: usize = index.value() as usize;
      let tmp = unsafe {
         Bkpr(::core::ptr::read_volatile((self.base() + 0x50 + (index << 2)) as *const u32))
