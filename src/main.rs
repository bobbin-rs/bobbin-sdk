...
fn write_device<W: Write>(cfg: &mut Config<W>, depth: usize, d: &Device) -> std::io::Result<()> {
    if cfg.compact {
        ...
        if let Some(ref size) = d.size {
            try!(write!(&mut cfg.out, " (size {})", size));
        }
...
fn write_peripheral<W: Write>(cfg: &mut Config<W>,
                              depth: usize,
                              d: &Peripheral)
                              -> std::io::Result<()> {
    if cfg.compact {
...
fn write_interrupt<W: Write>(cfg: &mut Config<W>,
                             depth: usize,
                             d: &Interrupt)
                             -> std::io::Result<()> {
    if cfg.compact {
...
fn write_register<W: Write>(cfg: &mut Config<W>,
                            depth: usize,
                            d: &Register)
                            -> std::io::Result<()> {
    if cfg.compact {
...
fn write_enumerated_value<W: Write>(cfg: &mut Config<W>,
                                    depth: usize,
                                    d: &EnumeratedValue)
                                    -> std::io::Result<()> {
    if cfg.compact {
