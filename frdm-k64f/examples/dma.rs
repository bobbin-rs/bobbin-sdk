...
pub extern "C" fn main() -> ! {
    ...
    d.set_tcd_citer_elinkno(ch.index(), TcdCiterElinkno(0x0001));
    d.set_tcd_biter_elinkno(ch.index(), TcdBiterElinkno(0x0001));
    d.set_tcd_nbytes_mlno(ch.index(), TcdNbytesMlno(32));

    d.set_tcd_saddr(ch.index(), TcdSaddr(0).set_saddr(src.as_ptr() as u32));
    d.set_tcd_soff(ch.index(), TcdSoff(0).set_soff(0x1));
    d.with_tcd_attr(ch.index(), |r| r.set_ssize(0));
    d.set_tcd_slast(ch.index(), TcdSlast(0));
    


    d.set_tcd_daddr(ch.index(), TcdDaddr(0).set_daddr(dst.as_mut_ptr() as u32));
    d.set_tcd_doff(ch.index(), TcdDoff(0).set_doff(0x1));
    d.with_tcd_attr(ch.index(), |r| r.set_dsize(0).set_dmod(0).set_smod(0));
    d.set_tcd_dlastsga(ch.index(), TcdDlastsga(0));
